...
{
    public class frmDM_LyDoTraHang : frmDM_ListBaseNew
    {
        ...
        public frmDM_LyDoTraHang()
        {
            InitializeComponent();
            SyncProvider = DMLyDoTraHangDataProvider.Instance;
            LoadSync();
        }
        private void Delete()
        {
            DMLyDoTraHangDataProvider.Instance.Delete(new DMLyDoTraHangInfo() { IdLyDoTH = Oid });
            LoadData();
            SetControl(false);
        }
        protected override void LoadData()
        {
            grcBase.DataSource = DMLyDoTraHangDataProvider.Instance.GetListLyDoTraHangInfo();
        ...
        private void frmDM_LyDoTraHang_OnThemMoi(object sender, EventArgs e)
        {
            isAdd = true;
            frmChiTiet_LyDoTraHang frm = new frmChiTiet_LyDoTraHang(this);
            frm.ShowDialog();
        }

        private void frmDM_LyDoTraHang_OnCapNhat(object sender, EventArgs e)
        {
            if (Oid > 0)
            {
                isAdd = false;
        ...
        void frmDM_LyDoTraHang_OnGridCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            SetControl(true);
            Oid = Convert.ToInt32(((DMLyDoTraHangInfo)dgvDanhSachMatHang.GetFocusedRow()).IdLyDoTH.ToString());
        }

        private void frmDM_LyDoTraHang_OnGridDoubleClick(object sender, EventArgs e)
        {
            if (Oid > 0)
            {
                isAdd = false;
        ...
        private void frmDM_LyDoTraHang_OnXoa(object sender, EventArgs e)
        {
            Delete();
        }
        ...
        {
            grcBase.DataSource =
                DMLyDoTraHangDataProvider.Instance.Search(new DMLyDoTraHangInfo { MaLyDo = txtTimKiemMa.Text, Ten = txtTimKiemTen.Text });
        }
